                ...
                <div class=" mb-4 sm:mb-5 flex flex-col gap-4">
                    <div class="">
                        ...
                        <input type="text" name="title" id="title"
                               ...
                               value="{{old('title')}}" placeholder="Type post title" autofocus>
                        @error('title')
                    ...
                    <div>
                        ...
                        <select
                            ...
                            @foreach($categories as $category)
                                <option value="{{old('category_id') ?? $category->id}}" @selected(old('category_id') == $category->id)>{{$category->name}}</option>
                            @endforeach
                        </select>
                        @error('category_id')
                        <span class="text-red-500 mt-2 block">{{$message}}</span>
                    ...
                    <div class="">
                        ...
                        <textarea name="body" id="body" rows="8"
                                  ...
                                  placeholder="Write a Body Post description here...">{{old('body')}}</textarea>
                        @error('body')
                        <span class="text-red-500 mt-2 block">{{$message}}</span>
                        @enderror
                    </div>
